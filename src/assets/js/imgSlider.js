...
// window.onload = load;
	...
	window.img = {
		...
		imagebg_li_small: document.getElementById("small_pic").getElementsByTagName("li"),
		...
		init: function () {
			...
			for (var i = 0; i < img.imagebg_li.length; i++) {
				console.log(img.imagebg_li[i] + i + " is OK")
				if (i != 0) {
					img.small_pic_html.push("<li onclick='window.img.play(" + i + ")'><div class='meng-ceng'></div><img src='" + img.imagebg_li[i].getAttribute("data-sPic") + "' /><p class='img-small-text'>"+ img.imagebg_li[i].getAttribute('data-text') +"</p></li>");
					img.imagebg.getElementsByTagName("li")[i].style.display = "none";
				} else {
					img.small_pic_html.push("<li onclick='window.img.play(" + i + ")' class='presently'><div class='meng-ceng'></div><img src='" + img.imagebg_li[i].getAttribute("data-sPic") + "' /><p class='img-small-text'>"+ img.imagebg_li[i].getAttribute('data-text') +"</p></li>");
				}
				//console.log(img.small_pic_html);
			}
			//console.log(img.small_pic_html)
			img.small_pic.innerHTML = img.small_pic_html.join("");
			img.present.style.left = "29px";
			...
			for (let i = 0; i < img.imagebg_li.length; i++) {
				img.imagebg_li_small[i].onmousemove = function () {
					img.imagebg_li_small[i].getElementsByTagName('div')[0].style.opacity = 0;
				}
				img.imagebg_li_small[i].onmouseout = function () {
					img.imagebg_li_small[i].getElementsByTagName('div')[0].style.opacity = 0.5;
				}
			}
		...
		play: function (i,p) {
			for (let i = 0; i < img.imagebg_li.length; i++) {
				img.imagebg_li[i].getElementsByTagName('video')[0].pause(); //暂停视频
			}
			...
			if (i > img.num) {
				...
				if (parseInt(img.present.style.left) > 300) {
					//如果活动框到了最右边--完成
					//小图片向左
